...
begin
  ...
  if debt_row.current_balance is not distinct from target_current_balance and debt_row.is_active is not distinct from target_is_active and debt_row.paid_off_at is not distinct from target_paid_off_at then raise exception 'No debt fields were changed.'; end if;
  ...
  update public.debts set current_balance = target_current_balance, is_active = target_is_active, paid_off_at = target_paid_off_at where id = target_debt_id returning * into debt_row;
