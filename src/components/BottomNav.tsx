...
const tabs = [
  {
    label: 'Home',
    href: '/dashboard',
    match: (p: string) => p === '/dashboard' || p.startsWith('/dashboard/monthly-check-in'),
    icon: (active: boolean) => (
  ...
  },
  {
    ...
    match: (p: string) => p.startsWith('/dashboard/plan') || p.startsWith('/dashboard/audit') || p.startsWith('/dashboard/adjust') || p.startsWith('/dashboard/life-events'),
    icon: (active: boolean) => (
      <svg className={`w-5 h-5 ${active ? 'text-emerald-600' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  },
  {
    ...
    match: (p: string) => p.startsWith('/dashboard/actuals') || p.startsWith('/dashboard/bonus-log') || p.startsWith('/dashboard/debts') || p.startsWith('/dashboard/net-worth') || p.startsWith('/dashboard/goals') || p.startsWith('/dashboard/investments') || p.startsWith('/dashboard/cash-flow') || p.startsWith('/dashboard/logs'),
    icon: (active: boolean) => (
      <svg className={`w-5 h-5 ${active ? 'text-emerald-600' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
