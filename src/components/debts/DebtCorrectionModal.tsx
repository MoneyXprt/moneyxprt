...
export function DebtCorrectionModal({ debt, onClose, onSubmit }: DebtCorrectionModalProps) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setError(null);
    try {
      await onSubmit({ currentBalance: Number(balance.replace(/,/g, '')), isActive }, reason);
      onClose();
    } catch (caughtError) {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end bg-gray-950/50 sm:items-center sm:justify-center sm:p-4" role="presentation">
      <form onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="debt-correction-title" className="w-full rounded-t-2xl bg-white p-5 shadow-xl sm:max-w-md sm:rounded-2xl">
        ...
        <label className="mt-5 block text-sm font-medium text-gray-700">Current balance<input type="text" inputMode="decimal" required value={balance} onChange={event => setBalance(event.target.value.replace(/[^0-9.]/g, ''))} className="mt-1.5 min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" /></label>
        ...
        <p className="mt-2 text-xs text-gray-500">Original balance: {formatDebtUsd(debt.originalBalance)}. Marking paid off sets the balance to $0.</p>
