...
function generatePDF(data: CpaReportData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const { taxpayerSummary, implementedStrategies, repsSummary, totalEstimatedSavings } = data;
    ...
    if (implementedStrategies.length > 0) {
      ...
      for (let i = 0; i < implementedStrategies.length; i++) {
        const s = implementedStrategies[i];
        ...
        if (i > 0 && doc.y + neededH > doc.page.height - L) {
          doc.addPage();
        ...
        if (i < implementedStrategies.length - 1) {
          doc.moveDown(0.3);
          rule('#e5e7eb');
          doc.moveDown(0.8);
        }
