...
export function DebtCorrectionModal({ debt, onClose, onSubmit }: DebtCorrectionModalProps) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setError(null);
    try {
      await onSubmit({ originalBalance: Number(originalBalance.replace(/,/g, '')), currentBalance: Number(balance.replace(/,/g, '')), interestRate: Number(interestRate.replace(/,/g, '')), minimumPayment: Number(minimumPayment.replace(/,/g, '')), isActive }, reason);
      onClose();
    } catch (caughtError) {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end bg-gray-950/50 sm:items-center sm:justify-center sm:p-4" role="presentation">
      <form onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="debt-correction-title" className="w-full rounded-t-2xl bg-white p-5 shadow-xl sm:max-w-md sm:rounded-2xl">
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2"><label className="block text-sm font-medium text-gray-700">Original balance<input type="text" inputMode="decimal" required value={originalBalance} onChange={event => setOriginalBalance(event.target.value.replace(/[^0-9.]/g, ''))} className="mt-1.5 min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" /></label><label className="block text-sm font-medium text-gray-700">Current balance<input type="text" inputMode="decimal" required value={balance} onChange={event => setBalance(event.target.value.replace(/[^0-9.]/g, ''))} className="mt-1.5 min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" /></label><label className="block text-sm font-medium text-gray-700">Interest rate<input type="text" inputMode="decimal" required value={interestRate} onChange={event => setInterestRate(event.target.value.replace(/[^0-9.]/g, ''))} className="mt-1.5 min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" /></label><label className="block text-sm font-medium text-gray-700">Minimum monthly payment<input type="text" inputMode="decimal" required value={minimumPayment} onChange={event => setMinimumPayment(event.target.value.replace(/[^0-9.]/g, ''))} className="mt-1.5 min-h-11 w-full rounded-lg border border-gray-300 px-3 text-sm" /></label></div>
